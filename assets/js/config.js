...
const CONFIG = {
  ...
  contacts: {
    ...
    email: "info@oceanfreshcaviar.com", // пересылка на рабочий ящик; null — блок скрыт
  ...
  /* ---------- 2. КУДА УХОДИТ ЗАКАЗ ----------
     ...
     Касается только телефонов: там sms открывает сообщение с готовым
     текстом заказа. На компьютере канал выбирается сам, по надёжности —
     сначала чат, потом письмо, потом буфер обмена, — поэтому ставить сюда
     "email" не нужно: это переключило бы на письмо и телефоны, где
     сообщение работает лучше. */
  primaryChannel: "sms",
  ...
  products: [
    {
      ...
      variants: [
        { grams: 125,  price: 100, inStock: false },
        { grams: 250,  price: 180, inStock: false },
        { grams: 500,  price: 350, inStock: false },
    ...
    },
    {
      ...
      variants: [
        { grams: 125,  price: 125, inStock: false },
        { grams: 250,  price: 240, inStock: false },
        { grams: 500,  price: 450, inStock: false },
        { grams: 1000, price: 0,   inStock: false, wholesale: true },
      ],
    },
    {
  ...
  /* ---------- 5b. Оповещения о событиях на сайте --------------------------
     Адрес эндпоинта своего Worker. Пока null — маячок молчит и ничего
     никуда не шлёт.
       url:     "https://oceanfresh-chat.canalkrosh.workers.dev/event"
       quietMs: пауза после последнего клика по корзине, мс.
                Корзину набирают в несколько кликов — ждём тишины и шлём
                один пуш вместо трёх. По умолчанию 90 секунд.
     Куда именно приходит уведомление, решают секреты Worker, не этот файл.

     Свои заходы глушатся на устройстве: открыть сайт с ?mute=1 один раз,
  ...
  /* ---------- 6b. Галерея настоящих фото ----------------------------------
     Только реальные снимки: как приезжает коробка, вскрытая банка, доставка.
     Студийные и стоковые кадры сюда не кладём — смысл блока именно в том,
     что это снято нами, а не куплено.
     Файлы -> assets/img/gallery/, длинная сторона 1600 px, до ~250 КБ.
     Пустой массив = блок на сайте не показывается.

       { src: "assets/img/gallery/box.jpg",
         alt:     { ru: "Коробка с икрой и гелевыми пакетами",
                    en: "Cooler with tins and gel packs" },
         caption: { ru: "Так уезжает заказ: термокороб, гелевые пакеты, крафт.",
                    en: "How an order leaves us: cooler, gel packs, kraft padding." } },
  ------------------------------------------------------------------------ */
  gallery: [],
