(() => {
  ...
  document.getElementById("orderForm").addEventListener("submit", (e) => {
    ...
    /* На телефоне ссылка sms: открывает сообщение, и этого достаточно.
       На компьютере она не открывает ничего, и человек остаётся с
       ощущением сломанной кнопки. Порядок запасных путей на компьютере
       выбран по надёжности, а не по красоте:

         1. чат — работает всегда, когда загрузился, и там отвечает бот;
         2. mailto: — настоящий вариант, но у людей с веб-почтой без
            настроенного клиента он так же молча не срабатывает, поэтому
            он второй, а не первый;
         3. буфер обмена и номер на экране — не срабатывать не умеет.   */
    if (!onPhone()) {
